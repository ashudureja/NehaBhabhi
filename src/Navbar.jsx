...
const Navbar = ({ isContentVisible }) => {
  ...
  return (
    ...
    >
      <div className='h-[50px] w-[50px] -ml-4'>
        <img className='rounded-[10px] h-full w-full object-cover' src="./bhb.gif" />
      </div>

      {tags.map((item, i) => (
        <h2 key={i} className='text-[16px] font-[f6] tracking-wider text-white font-light'>
          {item}
        </h2>
      ))}
    </motion.nav>
